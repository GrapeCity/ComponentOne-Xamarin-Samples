        ...
        private async Task GenerateRandomChangeAsync()
        {
            ...
            switch (_rand.Next(4))
            {
                ...
                case 0:
                    ...
                    if(_toggle) await grid.ColorTo(Color.Orange, C1ThemeInfo.ApplicationTheme.BackgroundColor, c => grid.Rows[random].BackgroundColor = c, ANIMATION_LENGTH);
                    break;
                case 1:
                ...
                case 2:
                    ...
                    if (_customers.Count > 0)
                    {
                        ...
                        if (random > moveto)
                        {
                            if(_toggle) await grid.ColorTo(Color.LightGreen, C1ThemeInfo.ApplicationTheme.BackgroundColor, c => grid.Rows[moveto].BackgroundColor = c, ANIMATION_LENGTH);
                        }
                        else if (random < moveto)
                        {
                            if(_toggle) await grid.ColorTo(Color.IndianRed, C1ThemeInfo.ApplicationTheme.BackgroundColor, c => grid.Rows[moveto].BackgroundColor = c, ANIMATION_LENGTH);
                        }
                    }
                    break;
                case 3:
                    if (_customers.Count > 0)
                    {
                        if (_toggle) grid.ColorTo(Color.LightBlue, C1ThemeInfo.ApplicationTheme.BackgroundColor, c => grid.Rows[random].BackgroundColor = c, ANIMATION_LENGTH);
